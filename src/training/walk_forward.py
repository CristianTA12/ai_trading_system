...
def training_folds(data: ExperimentData):
    ...
    for begin, end in zip(FOLD_BOUNDARIES[:-1], FOLD_BOUNDARIES[1:], strict=True):
        ...
        train = index[index + STEP < start]
        evaluation = index[(index >= start) & (index + STEP < stop)]
        ...
        if bars.empty or train.max() + STEP >= evaluation.min():
